...
def gnu_flag_available(x):
    return False

pymake.pymake.flag_available = gnu_flag_available('')
...
def make_mfnwt(dl=True,init_del=True,final_del=True,clean=True):
    ...
    try:
        pymake.main(srcdir, target, 'ifort', 'cl', makeclean=clean,
                    expedite=False, dryrun=False, double=False, debug=False,
                    fflags='fpp DSWR_OUTER_1 DRIP_ET DSYNC_SWR_RIPET fpe:0')
